...
import static io.jackey.search.RediSearchUtil.toStringMap;
...
//    Pipeline p = new Pipeline(c);
    ...
    p.hset("doc1", RediSearchUtil.toStringMap(fields));
    p.hset("doc2", RediSearchUtil.toStringMap(fields));
...
//    Pipeline p = new Pipeline(c);
    ...
    p.jsonSet("arr", Path.ROOT_PATH, new int[]{ 0, 1, 2, 3 });
    Response<Object> pop = p.jsonArrPop("arr");
    Response<Long> arrLen = p.jsonArrLen("arr");
    p.jsonSet("baz", Path.ROOT_PATH, new Baz[]{ baz1, baz2, baz3 });
