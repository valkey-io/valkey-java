...
import io.jackey.util.ByteArrayUtil;
...
public class SetPipelineCommandsTest extends PipelineCommandsTestBase {
  ...
  public void spopWithCount() {
    ...
    AssertUtil.assertCollectionContainsAll(superSet, members1.get());
    ...
    AssertUtil.assertByteArrayCollectionContainsAll(bsuperSet, bmembers1.get());
    ByteArrayUtil.byteArrayCollectionRemoveAll(bsuperSet, bmembers1.get());

    AssertUtil.assertByteArraySetEquals(bsuperSet, bmembers2.get());
  }
  ...
  public void sscan() {
    ...
    Response<ScanResult<String>> result = pipe.sscan("foo", ScanParams.SCAN_POINTER_START);

    pipe.sync();

    assertThat(result.get().getCursor(), equalTo(ScanParams.SCAN_POINTER_START));
    assertThat(result.get().getResult(), not(empty()));

    // binary
    pipe.sadd(bfoo, ba, bb);

    Response<ScanResult<byte[]>> bResult = pipe.sscan(bfoo, ScanParams.SCAN_POINTER_START_BINARY);

    pipe.sync();

    assertThat(bResult.get().getCursor(), equalTo(ScanParams.SCAN_POINTER_START));
  ...
  public void sscanMatch() {
    ...
    Response<ScanResult<String>> result = pipe.sscan("foo", ScanParams.SCAN_POINTER_START, params);
    ...
    Response<ScanResult<byte[]>> bResult = pipe.sscan(bfoo, ScanParams.SCAN_POINTER_START_BINARY, params);
