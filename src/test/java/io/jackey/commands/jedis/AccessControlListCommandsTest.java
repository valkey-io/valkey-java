...
import io.jackey.util.RedisVersionUtil;
...
public class AccessControlListCommandsTest extends JedisCommandsTestBase {
  ...
  public void aclWhoAmI() {
    ...
    Assert.assertArrayEquals(SafeEncoder.encode("default"), binary);
  }
  ...
  public void aclLogTest() {
    ...
    Assert.assertEquals(10, jedis.aclLog().get(0).getCount());
    Assert.assertEquals("get", jedis.aclLog().get(0).getObject());
    ...
    Assert.assertEquals(1, jedis.aclLog().get(0).getCount());
    Assert.assertEquals("somekeynotallowed", jedis.aclLog().get(0).getObject());
    Assert.assertEquals("key", jedis.aclLog().get(0).getReason());
    ...
    assertEquals("Number of log messages ", 1, jedis.aclLog().size());
    Assert.assertEquals(1, jedis.aclLog().get(0).getCount());
    Assert.assertEquals("multi", jedis.aclLog().get(0).getContext());
    Assert.assertEquals("incr", jedis.aclLog().get(0).getObject());
