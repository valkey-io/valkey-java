...
public class ClusterBinaryValuesCommandsTest extends ClusterJedisCommandsTestBase {
  ...
  public void georadiusStoreBinary() {
      ...
      AssertUtil.assertByteArrayListEquals(bexpected, cluster.zrange("{Sicily}Store".getBytes(), 0, -1));
  }
  ...
  public void testBinaryGeneralCommand() {
    ...
    cluster.sendCommand("z".getBytes(), Command.SET, key, value);
    cluster.sendCommand("y".getBytes(), Command.INCR, key);
    Object returnObj = cluster.sendCommand("w".getBytes(), Command.GET, key);
    Assert.assertEquals("2", SafeEncoder.encode((byte[]) returnObj));
  }

  @Test
  public void testGeneralCommand() {
    cluster.sendCommand("z", Command.SET, "x", "1");
    cluster.sendCommand("y", Command.INCR, "x");
    Object returnObj = cluster.sendCommand("w", Command.GET, "x");
