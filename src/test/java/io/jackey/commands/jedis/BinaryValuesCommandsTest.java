...
public class BinaryValuesCommandsTest extends JedisCommandsTestBase {
  ...
  public void setNxExAndGet() {
    assertEquals("OK", jedis.set(bfoo, binaryValue, SetParams.setParams().nx().ex(expireSeconds)));
  ...
  public void setIfNotExistAndGet() {
    ...
    assertNull(jedis.set(bfoo, binaryValue, SetParams.setParams().nx().ex(expireSeconds)));
  ...
  public void setIfExistAndGet() {
    assertEquals("OK", jedis.set(bfoo, binaryValue));
    // nx should fail if value exists
    assertEquals("OK", jedis.set(bfoo, binaryValue, SetParams.setParams().xx().ex(expireSeconds)));
  ...
  public void setFailIfNotExistAndGet() {
    ...
    assertNull(jedis.set(bfoo, binaryValue, SetParams.setParams().xx().ex(expireSeconds)));
  }

  @Test
  public void setAndExpireMillis() {
    assertEquals("OK", jedis.set(bfoo, binaryValue, SetParams.setParams().nx().px(expireMillis)));
  ...
  public void setAndKeepttl() {
    assertEquals("OK", jedis.set(bfoo, binaryValue, SetParams.setParams().nx().ex(expireSeconds)));
    assertEquals("OK", jedis.set(bfoo, binaryValue, SetParams.setParams().keepttl()));
    assertEquals("OK", jedis.set(bfoo, binaryValue, SetParams.setParams().keepTtl()));
    long ttl = jedis.ttl(bfoo);
  ...
  public void setAndPxat() {
    assertEquals("OK", jedis.set(bfoo, binaryValue,
        SetParams.setParams().nx().pxAt(System.currentTimeMillis() + expireMillis)));
    long ttl = jedis.ttl(bfoo);
    assertTrue(ttl > 0 && ttl <= expireSeconds);
  }
  ...
  public void setAndExat() {
    assertEquals("OK", jedis.set(bfoo, binaryValue,
        SetParams.setParams().nx().exAt(System.currentTimeMillis() / 1000 + expireSeconds)));
    long ttl = jedis.ttl(bfoo);
    assertTrue(ttl > 0 && ttl <= expireSeconds);
  }
  ...
  public void mget() {
    ...
    AssertUtil.assertByteArrayListEquals(expected, values);
    ...
    expected.add(null);
    AssertUtil.assertByteArrayListEquals(expected, jedis.mget(bfoo, bbar));
