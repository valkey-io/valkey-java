...
public class JedisURIHelperTest {
  ...
  public void shouldGetDefaultProtocolWhenNotDefined() {
    assertNull(JedisURIHelper.getRedisProtocol(URI.create("redis://host:1234")));
    assertNull(JedisURIHelper.getRedisProtocol(URI.create("redis://host:1234/1")));
  }

  @Test
  public void shouldGetProtocolFromDefinition() {
    Assert.assertEquals(RedisProtocol.RESP3, JedisURIHelper.getRedisProtocol(URI.create("redis://host:1234?protocol=3")));
    Assert.assertEquals(RedisProtocol.RESP3, JedisURIHelper.getRedisProtocol(URI.create("redis://host:1234/?protocol=3")));
    Assert.assertEquals(RedisProtocol.RESP3, JedisURIHelper.getRedisProtocol(URI.create("redis://host:1234/1?protocol=3")));
    Assert.assertEquals(RedisProtocol.RESP3, JedisURIHelper.getRedisProtocol(URI.create("redis://host:1234/1/?protocol=3")));
  }
}
