...
public abstract class BitCommandsTestBase extends UnifiedJedisCommandsTestBase {
  ...
  public void bitposModifier() {
    ...
    Assert.assertEquals(0, jedis.bitpos("mykey", false));
    Assert.assertEquals(1, jedis.bitpos("mykey", true));
    Assert.assertEquals(1, jedis.bitpos("mykey", true, BitPosParams.bitPosParams()));
    Assert.assertEquals(18, jedis.bitpos("mykey", true, BitPosParams.bitPosParams().start(2)));
    Assert.assertEquals(18, jedis.bitpos("mykey", true, BitPosParams.bitPosParams().start(2).end(-1)));
    Assert.assertEquals(18, jedis.bitpos("mykey", true, BitPosParams.bitPosParams().start(2).end(-1)
        .modifier(BitCountOption.BYTE)));
    Assert.assertEquals(9, jedis.bitpos("mykey", true, BitPosParams.bitPosParams().start(7).end(15)
  ...
  public void setAndgetrange() {
    ...
    Assert.assertEquals("Hello Jedis", jedis.get("key1"));
  ...
  public void bitOpBinary() {
    ...
    Assert.assertArrayEquals(new byte[]{0x2}, jedis.get(dest));

    jedis.bitop(BitOP.OR, dest, key1, key2);
    Assert.assertArrayEquals(new byte[]{0x7}, jedis.get(dest));

    jedis.bitop(BitOP.XOR, dest, key1, key2);
    Assert.assertArrayEquals(new byte[]{0x5}, jedis.get(dest));

    jedis.setbit(key1, 0, true);
    jedis.bitop(BitOP.NOT, dest, key1);
    Assert.assertArrayEquals(new byte[]{0x79}, jedis.get(dest));
  }
