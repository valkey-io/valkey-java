...
//  }
  ...
  public void addSuggestionAndGetSuggestion() {
    ...
    Assert.assertEquals(suggestion + " did not get a match with 3 characters",
        1, client.ftSugGet(key, suggestion.substring(0, 3), true, 5).size());

    // turn off fuzzy start at second word no hit
    Assert.assertEquals(noMatch + " no fuzzy and starting at 1, should not match",
        0, client.ftSugGet(key, noMatch.substring(1, 6), false, 5).size());

    // my attempt to trigger the fuzzy by 1 character
    Assert.assertEquals(noMatch + " fuzzy is on starting at 1 position should match",
  ...
  public void addSuggestionIncrAndGetSuggestionFuzzy() {
    ...
    Assert.assertEquals(suggestion, client.ftSugGet(key, suggestion.substring(0, 3)).get(0));
  }
  ...
  public void getSuggestionMax() {
    ...
    // test that with a partial part of that string will have the entire word returned
    Assert.assertEquals("3 suggestions", 3, client.ftSugGetWithScores(key, "COU", true, 10).size());
    Assert.assertEquals(2, client.ftSugGetWithScores(key, "COU", true, 2).size());
  }
  ...
  public void getSuggestionNoHit() {
    ...
    Assert.assertEquals(emptyList(), client.ftSugGetWithScores(key, "DIF"));
    Assert.assertEquals(emptyList(), client.ftSugGet(key, "DIF"));
  }
  ...
  public void getSuggestionLengthAndDeleteSuggestion() {
    ...
    Assert.assertEquals(2L, client.ftSugLen(key));

    assertTrue("Delete suggestion should succeed.", client.ftSugDel(key, "ANOTHER ENTRY"));
    Assert.assertEquals(1L, client.ftSugLen(key));
