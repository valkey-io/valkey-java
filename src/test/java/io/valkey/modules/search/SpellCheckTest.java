...
//  }
  ...
  public void dictionary() {
    Assert.assertEquals(3L, client.ftDictAdd("dict", "foo", "bar", "hello world"));
    Assert.assertEquals(new HashSet<>(Arrays.asList("foo", "bar", "hello world")), client.ftDictDump("dict"));
    Assert.assertEquals(3L, client.ftDictDel("dict", "foo", "bar", "hello world"));
    Assert.assertEquals(Collections.emptySet(), client.ftDictDump("dict"));
  }

  @Test
  public void dictionaryBySampleKey() {
    Assert.assertEquals(3L, client.ftDictAddBySampleKey(index, "dict", "foo", "bar", "hello world"));
    Assert.assertEquals(new HashSet<>(Arrays.asList("foo", "bar", "hello world")),
        client.ftDictDumpBySampleKey(index, "dict"));
    Assert.assertEquals(3L, client.ftDictDelBySampleKey(index, "dict", "foo", "bar", "hello world"));
    Assert.assertEquals(Collections.emptySet(), client.ftDictDumpBySampleKey(index, "dict"));
  }
  ...
  public void crossTermDictionary() {
    ...
    Assert.assertEquals(expected, client.ftSpellCheck(index, "Tooni toque kerfuffle",
