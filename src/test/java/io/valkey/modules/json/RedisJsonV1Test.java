package io.valkey.modules.json;
...
import io.valkey.json.commands.RedisJsonV1Commands;
import io.valkey.modules.RedisModuleCommandsTestBase;
import io.valkey.modules.json.JsonObjects.Baz;
import io.valkey.modules.json.JsonObjects.FooBarObject;
import io.valkey.modules.json.JsonObjects.IRLObject;
import io.valkey.modules.json.JsonObjects.Person;
import io.valkey.modules.json.JsonObjects.Qux;
import io.valkey.modules.json.JsonObjects.Tick;
import io.valkey.util.JsonObjectMapperTestUtil;
...
//    jsonClient.jsonSet("null", null, ROOT_PATH);
    jsonV1.jsonSet("null", Path.ROOT_PATH, (Object) null);
    assertNull(jsonV1.jsonGet("null", String.class, Path.ROOT_PATH));

    // real scalar value and no path
    jsonV1.jsonSet("str", Path.ROOT_PATH, "strong");
    assertEquals("strong", jsonV1.jsonGet("str"));

    // a slightly more complex object
    IRLObject obj = new IRLObject();
    jsonV1.jsonSet("obj", Path.ROOT_PATH, obj);
  ...
  public void setExistingPathOnlyIfExistsShouldSucceed() {
    jsonV1.jsonSet("obj", Path.ROOT_PATH, new IRLObject());
    Path p = Path.of(".str");
  ...
  public void setWithoutAPathDefaultsToRootPath() {
    jsonV1.jsonSet("obj1", Path.ROOT_PATH, new IRLObject());
//    jsonClient.jsonSet("obj1", "strangle", JsonSetParams.jsonSetParams().xx());
    ...
    assertEquals("strangle", jsonV1.jsonGet("obj1", String.class, Path.ROOT_PATH));
  }
  ...
  public void getAbsent() {
    jsonV1.jsonSet("test", Path.ROOT_PATH, "foo");
  ...
  public void delNonExistingPathsAreIgnored() {
    jsonV1.jsonSet("foobar", Path.ROOT_PATH, new FooBarObject());
  ...
  public void typeChecksShouldSucceed() {
    ...
    assertSame(Object.class, jsonV1.jsonType("foobar", Path.ROOT_PATH));
  ...
  public void arrAppendSameType() {
    ...
    jsonV1.jsonSet("test_arrappend", Path.ROOT_PATH, jsonObject);
  ...
  public void arrIndexAbsentKey() {
    jsonV1.jsonArrIndex("quxquux", Path.ROOT_PATH, gson.toJson(new Object()));
  }

  @Test
  public void arrIndexWithInts() {
    jsonV1.jsonSet("quxquux", Path.ROOT_PATH, new int[]{8, 6, 7, 5, 3, 0, 9});
    assertEquals(2L, jsonV1.jsonArrIndex("quxquux", Path.ROOT_PATH, 7));
    assertEquals(-1L, jsonV1.jsonArrIndex("quxquux", Path.ROOT_PATH, "7"));
  }

  @Test
  public void arrIndexWithStrings() {
    jsonV1.jsonSet("quxquux", Path.ROOT_PATH, new String[]{"8", "6", "7", "5", "3", "0", "9"});
    assertEquals(2L, jsonV1.jsonArrIndex("quxquux", Path.ROOT_PATH, "7"));
  }
  ...
  public void arrInsert() {
    ...
    jsonV1.jsonSet("test_arrinsert", Path.ROOT_PATH, jsonArray);
    Assert.assertEquals(8L, jsonV1.jsonArrInsert("test_arrinsert", Path.ROOT_PATH, 1, "foo"));

    Object[] array = jsonV1.jsonGet("test_arrinsert", Object[].class, Path.ROOT_PATH);
  ...
  public void testArrayPop() {
    jsonV1.jsonSet("arr", Path.ROOT_PATH, new int[]{0, 1, 2, 3, 4});
    assertEquals(Long.valueOf(4), jsonV1.jsonArrPop("arr", Long.class, Path.ROOT_PATH));
    assertEquals(Long.valueOf(3), jsonV1.jsonArrPop("arr", Long.class, Path.ROOT_PATH, -1));
    assertEquals(Long.valueOf(2), jsonV1.jsonArrPop("arr", Long.class));
    assertEquals(Long.valueOf(0), jsonV1.jsonArrPop("arr", Long.class, Path.ROOT_PATH, 0));
  ...
  public void arrTrim() {
    jsonV1.jsonSet("arr", Path.ROOT_PATH, new int[]{0, 1, 2, 3, 4});
    assertEquals(Long.valueOf(3), jsonV1.jsonArrTrim("arr", Path.ROOT_PATH, 1, 3));
    assertArrayEquals(new Integer[]{1, 2, 3}, jsonV1.jsonGet("arr", Integer[].class, Path.ROOT_PATH));
  }

  @Test
  public void strAppend() {
    jsonV1.jsonSet("str", Path.ROOT_PATH, "foo");
    assertEquals(6L, jsonV1.jsonStrAppend("str", Path.ROOT_PATH, "bar"));
    assertEquals("foobar", jsonV1.jsonGet("str", String.class, Path.ROOT_PATH));
...
//    assertEquals("foobared", jsonClient.jsonGet("str", String.class));
  ...
  public void strLen() {
    ...
    assertEquals(Long.valueOf(3), jsonV1.jsonStrLen("str", Path.ROOT_PATH));
  }
  ...
  public void obj() {
    ...
    assertNull(jsonV1.jsonObjLen("doc", Path.ROOT_PATH));
    assertNull(jsonV1.jsonObjKeys("doc", Path.ROOT_PATH));

    String json = "{\"a\":[3], \"nested\": {\"a\": {\"b\":2, \"c\": 1}}}";
    jsonV1.jsonSetWithPlainString("doc", Path.ROOT_PATH, json);
  ...
  public void debugMemory() {
    ...
    assertEquals(0L, jsonV1.jsonDebugMemory("json", Path.ROOT_PATH));

    String json = "{ foo: 'bar', bar: { foo: 10 }}";
    JsonObject jsonObject = gson.fromJson(json, JsonObject.class);
    jsonV1.jsonSet("json", Path.ROOT_PATH, jsonObject);
    // it is okay as long as any 'long' is returned
    jsonV1.jsonDebugMemory("json");
    jsonV1.jsonDebugMemory("json", Path.ROOT_PATH);
  ...
  public void plainString() {
    String json = "{\"foo\":\"bar\",\"bar\":{\"foo\":10}}";
    assertEquals("OK", jsonV1.jsonSetWithPlainString("plain", Path.ROOT_PATH, json));
    assertEquals(json, jsonV1.jsonGetAsPlainString("plain", Path.ROOT_PATH));
  }
  ...
  public void testJsonGsonParser() {
    ...
    jsonV1.jsonSet(person.getId(), Path.ROOT_PATH, person);
  ...
  public void testDefaultJsonGsonParserStringsMustBeDifferent() {
    ...
    jsonV1.jsonSet(tick.getId(), Path.ROOT_PATH, tick);
