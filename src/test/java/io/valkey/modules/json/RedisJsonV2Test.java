...
import static io.valkey.modules.json.JsonObjects.*;
...
import io.valkey.json.commands.RedisJsonV2Commands;
...
public class RedisJsonV2Test extends RedisModuleCommandsTestBase {
  ...
  public void basicSetGetShouldSucceed() {
    ...
    jsonV2.jsonSetWithEscape("null", Path2.ROOT_PATH, (Object) null);
    assertJsonArrayEquals(jsonArray((Object) null), jsonV2.jsonGet("null", Path2.ROOT_PATH));
...
//    jsonClient.jsonSet("obj1", "strangle", JsonSetParams.jsonSetParams().xx());
    ...
    assertJsonArrayEquals(jsonArray("strangle"), jsonV2.jsonGet("obj1", Path2.ROOT_PATH));
  }
  ...
  public void getAbsent() {
    jsonV2.jsonSetWithEscape("test", Path2.ROOT_PATH, "foo");
  ...
  public void delValidShouldSucceed() {
    ...
    jsonV2.jsonSetWithEscape("obj", Path2.ROOT_PATH, new IRLObject());
  ...
  public void delNonExistingPathsAreIgnored() {
    jsonV2.jsonSetWithEscape("foobar", Path2.ROOT_PATH, new FooBarObject());
    assertEquals(0L, jsonV2.jsonDel("foobar", Path2.of(".foo[1]")));
  }

  @Test
  public void typeChecksShouldSucceed() {
    jsonV2.jsonSet("foobar", Path2.ROOT_PATH, new JSONObject(gson.toJson(new FooBarObject())));
    assertEquals(singletonList(Object.class), jsonV2.jsonType("foobar", Path2.ROOT_PATH));
  ...
  public void testJsonMerge() {
    ...
    // cleanup
    Assert.assertEquals(1L, client.del("test_merge"));
  }
  ...
  @Test
  public void arrLen() {
    jsonV2.jsonSet("arr", Path2.ROOT_PATH, new JSONArray(new int[]{0, 1, 2, 3, 4}));
    assertEquals(singletonList(5L), jsonV2.jsonArrLen("arr", Path2.ROOT_PATH));
  }

  @Test
  public void clearArray() {
    jsonV2.jsonSet("foobar", Path2.ROOT_PATH, gson.toJson(new FooBarObject()));
...
//    assertEquals(new Baz(null, null, null), jsonClient.jsonGet("qux", objPath));
  ...
  public void arrIndexAbsentKey() {
    jsonV2.jsonArrIndexWithEscape("quxquux", Path2.ROOT_PATH, new JSONObject());
  }

  @Test
  public void arrIndexWithInts() {
    jsonV2.jsonSetWithEscape("quxquux", Path2.ROOT_PATH, new int[]{8, 6, 7, 5, 3, 0, 9});
    assertEquals(singletonList(2L), jsonV2.jsonArrIndexWithEscape("quxquux", Path2.ROOT_PATH, 7));
    assertEquals(singletonList(-1L), jsonV2.jsonArrIndexWithEscape("quxquux", Path2.ROOT_PATH, "7"));
  }

  @Test
  public void arrIndexWithStrings() {
    jsonV2.jsonSetWithEscape("quxquux", Path2.ROOT_PATH, new String[]{"8", "6", "7", "5", "3", "0", "9"});
    assertEquals(singletonList(2L), jsonV2.jsonArrIndexWithEscape("quxquux", Path2.ROOT_PATH, "7"));
  }
  ...
  public void arrInsert() {
    ...
    jsonV2.jsonSet("test_arrinsert", Path2.ROOT_PATH, new JSONArray(json));
    assertEquals(singletonList(8L), jsonV2.jsonArrInsertWithEscape("test_arrinsert", Path2.ROOT_PATH, 1, "foo"));

    assertJsonArrayEquals(jsonArray(jsonArray("hello", "foo", "world", true, 1, 3, null, false)),
        jsonV2.jsonGet("test_arrinsert", Path2.ROOT_PATH));
  }
  ...
  public void arrPop() {
    jsonV2.jsonSet("arr", Path2.ROOT_PATH, new JSONArray(new int[]{0, 1, 2, 3, 4}));
    assertEquals(singletonList(4d), jsonV2.jsonArrPop("arr", Path2.ROOT_PATH));
    assertEquals(singletonList(3d), jsonV2.jsonArrPop("arr", Path2.ROOT_PATH, -1));
    assertEquals(singletonList(0d), jsonV2.jsonArrPop("arr", Path2.ROOT_PATH, 0));
  }
...
//    jsonClient.jsonSet("arr", ROOT_PATH, new int[]{0, 1, 2, 3, 4});
    jsonV2.jsonSet("arr", Path2.ROOT_PATH, new JSONArray(new int[]{0, 1, 2, 3, 4}));
    assertEquals(singletonList(3L), jsonV2.jsonArrTrim("arr", Path2.ROOT_PATH, 1, 3));
//    assertArrayEquals(new Integer[]{1, 2, 3}, jsonClient.jsonGet("arr", Integer[].class, ROOT_PATH));
    assertJsonArrayEquals(jsonArray(jsonArray(1, 2, 3)), jsonV2.jsonGet("arr", Path2.ROOT_PATH));
  }

  @Test
  public void strAppend() {
//    jsonClient.jsonSet("str", ROOT_PATH, "foo");
    jsonV2.jsonSet("str", Path2.ROOT_PATH, gson.toJson("foo"));
    assertEquals(singletonList(6L), jsonV2.jsonStrAppend("str", Path2.ROOT_PATH, "bar"));
    assertJsonArrayEquals(jsonArray("foobar"), jsonV2.jsonGet("str", Path2.ROOT_PATH));
  }

  @Test
  public void strLen() {
    jsonV2.jsonSetWithEscape("str", "foobar");
    assertEquals(singletonList(6L), jsonV2.jsonStrLen("str", Path2.ROOT_PATH));
  }
  ...
  public void obj() {
    ...
    jsonV2.jsonSet("doc", Path2.ROOT_PATH, json);
    assertEquals(Arrays.asList(2L), jsonV2.jsonObjLen("doc", Path2.ROOT_PATH));
    assertEquals(Arrays.asList(Arrays.asList("a", "nested")), jsonV2.jsonObjKeys("doc", Path2.ROOT_PATH));
  ...
  public void debugMemory() {
    assertEquals(Collections.emptyList(), jsonV2.jsonDebugMemory("json", Path2.ROOT_PATH));

    jsonV2.jsonSet("json", new JSONObject("{ foo: 'bar', bar: { foo: 10 }}"));
    assertEquals(1, jsonV2.jsonDebugMemory("json", Path2.ROOT_PATH).size());
